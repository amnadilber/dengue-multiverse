...
coefficient \textbf{71\%} --- \textbf{42\%} when the epidemic is a sum of two
...
\item \textbf{The intervals do not keep their promise.} A nominal 95\% interval
      ...
      rainfall coefficient 71\% --- 42\% when the epidemic is a sum of two
...
structure, and the values at which the epidemiological parameters are fixed ---
none of which has an analogue in a covariate list, and each of which is normally
...
The design also lets us report something a specification curve cannot. A curve
orders specifications by their average result across datasets, which is a main
effect of the analysis; with many datasets analysed every way, the
dataset-by-specification interaction is separately estimable, and here it is the
largest of the three terms (Section~
ef{sec:decomposition}). Variance
decompositions over analytic \emph{factors} appear in the multiverse literature;
we have not found the interaction with the dataset reported, though the
multi-pipeline neuroimaging collections have the crossed design in which it could
be. It matters because it decides which remedy is available: a main effect can be
legislated away by convention and an interaction cannot.
...
The peak-prominence threshold is itself a choice, and Section~\ref{sec:checks}
...
\dot R_h &= \gamma_h I_h, &\dot C &= \sigma_h E_h,
\end{align*}
...
\paragraph{How much does this depend on which choices we called defensible?}
...
The optimiser check of Section~\ref{sec:checks} refits seven outbreaks from ten
...
check in Section~\ref{sec:checks} changes 0.6\% of verdicts under ten cold
...
\paragraph{The published moderator does not reproduce here.} The specific
...
epidemiological parameters --- lifespan, incubation periods, vector-to-host ratio
--- to the other end of their published ranges flips \textbf{3.1\%} and moves the
...
field uses it: the systematic review of 99 dengue prediction models finds
18.3\% using Poisson regression and 18.3\% linear regression, with the negative
binomial not appearing as a category of its own~\cite{review2023}, and
a head-to-head comparison on mosquito-borne outbreak data finds the negative
binomial the better of the two~\cite{nbcompare}. That is a large minority rather
than a majority, which is the honest reading and still enough that a multiverse
excluding Poisson would not describe the literature it is about. And the result does not depend on
...
Negative binomial only & 79.6\% & 75--85\% & --- & 100.0\% \\
Negative binomial, $|\Delta\mathrm{AIC}| > 10$ & 20.0\% & 14--25\% & --- & 74.7\% \\
...
outbreaks still receiving a verdict. The share where some pair disagrees (dark)
and the probability that two analyses disagree (green) both collapse between 2 and
...
\subsection{The instability is not specific to AIC, and stricter criteria are worse}
...
\subsection{Why: large movement, marginal dissent}
...
Applying the recommendation of Section~\ref{sec:remedy} to the 221 outbreaks ---
negative binomial only, abstaining where $|\Delta\mathrm{AIC}| < 4$:
...
That this matters in practice, and not only in principle, follows from what the
literature reports about its own habits. The systematic review of 99 dengue
prediction models finds 18.3\% using Poisson regression and 18.3\% linear
regression, with the negative binomial not appearing as a category of its
own~\cite{review2023}; where the two are compared head to head on mosquito-borne
outbreak data the negative binomial is the better model~\cite{nbcompare}. We do
not claim this is what most of the field does --- more than a third of that sample
is the defensible reading --- but the choice that raises the probability of
endorsing climate forcing from 0.57 to 0.90 is one a large minority makes, and one
that mechanistic fits, which the review does not cover, commonly make in the more
extreme form of ordinary least squares.
...
equivalent''.
That is the same dissociation reported in Section~4.3 from a different lever ---
in-sample endorsement of climate forcing runs at 68\% and out-of-sample at 35\%
...
\textbf{17} have no gap-free run of 30 weeks --- the mechanism the paragraph named
--- while \textbf{37} report continuously enough and are excluded because no wave
...
weaker~\cite{spatial2026}; Section~\ref{sec:checks} measures how often it changes
...
All analyses are reproducible from \url{https://github.com/amnadilber/dengue-multiverse}: 38 numbered pipeline
scripts, 266 tests, and a dated analysis log recording every formulation that
...
% ===========================================================================
\section*{Appendix: what can be identified at all, a worked case}
\label{sec:workedcase}
% ===========================================================================
